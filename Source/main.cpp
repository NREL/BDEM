...
int main( int argc, char *argv[] )
{
    ...
    IntVect domain_hi(
        AMREX_D_DECL( specs.ncells[XDIR] - 1, specs.ncells[YDIR] - 1, specs.ncells[ZDIR] - 1 ) );
    ...
    if ( !specs.restartedcase )
    {
        if ( specs.init_particles_using_file == 1 )
        {
            ...
            {
                bpc.InitBondedParticles(
                    "particle_input.dat",
                    specs.do_heat_transfer,
                    specs.contact_law,
                    specs.cantilever_beam_test,
        ...
        else
        {
            ...
            bpc.InitParticles(
                specs.autogen_mincoords.data(),
                specs.autogen_maxcoords.data(),
                specs.autogen_meanvel.data(),
                specs.autogen_fluctuation.data(),
                specs.autogen_radius,
                specs.autogen_dens,
                specs.autogen_E,
                specs.autogen_nu,
                specs.autogen_temp,
                ...
                specs.autogen_layer_particles,
                specs.bonded_sphere_particles,
                specs.autogen_min_radius,
                specs.autogen_max_radius,
                specs.autogen_bp_type,
                specs.particle_type_list,
                specs.use_type_dist,
                specs.particle_dist_list,
                specs.liquid_bridging,
                specs.liquid_density,
                specs.moisture_content,
                specs.moisture_content_stdev,
    ...
    else
    {
        ...
        if ( specs.reassign_particle_properties )
            bpc.reassignParticleProperties(
                specs.autogen_radius, specs.autogen_dens, specs.autogen_E, specs.autogen_nu );
    }

    if ( specs.coloring_striation )
        bpc.particleColoringStriation( specs.coloring_striation_len, specs.coloring_striation_dir );
    ...
    {
        bpc.InitChemSpecies(
            specs.nstrat_domains,
            specs.strat_mincoords.data(),
            specs.strat_maxcoords.data(),
            specs.strat_spec_massfracs.data() );
    }
    ...
    if ( !specs.restartedcase )
    {
        ...
        {
            bpc.removeParticlesOutsideBoundary(
                EBtools::lsphi, EBtools::ebfactory, EBtools::ls_refinement );
        ...
        {
            bpc.removeEBOverlapParticles(
                EBtools::ebfactory, EBtools::lsphi, EBtools::ls_refinement );
            amrex::Print() << "Num particles after EB overlap removal "
                           << bpc.TotalNumberOfParticles() << "\n";
        }
    }
    ...
    for ( int stli = 0; stli < specs.stls.size(); stli++ )
    {
        std::string stlpltfile = amrex::Concatenate(
            ( specs.stls[stli].name + "_" ).c_str(), steps + specs.stepoffset, 5 );
    ...
    if ( specs.liquid_bridging && specs.recalculate_MC )
        bpc.computeMoistureContent(
            specs.moisture_content, specs.moisture_content_stdev, specs.liquid_density, specs.FSP );
    ...
    if ( specs.calc_mass_flow )
        bpc.Calculate_Total_Mass_MaterialPoints(
            mass_flow_prev, specs.mass_flow_dir, specs.mass_flow_cutoff );
    ...
    while ( ( steps < specs.maxsteps ) and ( time < specs.final_time ) and
            ( total_speed > specs.avg_speed_stop ) )
    {
        time += dt;
        output_time += dt;
        output_timeMass += dt;
        ...
        if ( specs.cantilever_beam_test )
        {
            if ( specs.cb_intervals )
            {
                ...
                if ( temp_t > 0 && temp_t <= specs.cb_load_time && time < cb_int_stop )
                    cb_force +=
                        dt / ( specs.cb_interval_num * specs.cb_load_time ) * specs.cb_force_max;
                if ( temp_t > 0 && temp_t <= specs.cb_load_time && time < cb_int_stop )
                    cb_torq +=
                        dt / ( specs.cb_interval_num * specs.cb_load_time ) * specs.cb_torq_max;
            }
            else
            {
                cb_force = ( time < specs.cb_load_time )
                               ? ( time / specs.cb_load_time ) * specs.cb_force_max
                           : ( time < specs.cb_unload_time ) ? specs.cb_force_max
                                                             : 0.0;
                cb_torq  = ( time < specs.cb_load_time )
                               ? ( time / specs.cb_load_time ) * specs.cb_torq_max
                           : ( time < specs.cb_unload_time ) ? specs.cb_torq_max
                                                             : 0.0;
            }
        }
...

        {
            ...
            bpc.clearSourcingVolume(
                specs.particle_sourcing_mincoords.data(),
                specs.particle_sourcing_maxcoords.data() );
            bpc.InitParticles(
                specs.particle_sourcing_mincoords.data(),
                specs.particle_sourcing_maxcoords.data(),
                specs.particle_sourcing_meanvel.data(),
                specs.particle_sourcing_fluctuation.data(),
                specs.particle_sourcing_radius,
                specs.particle_sourcing_dens,
                specs.particle_sourcing_E,
                specs.particle_sourcing_nu,
                ...
                specs.bonded_sphere_particles,
                specs.particle_sourcing_min_radius,
                specs.particle_sourcing_max_radius,
            ...
            for ( int stli = 0; stli < specs.stls.size(); stli++ )
            {
                ...
                {
                    bpc.removeParticlesInsideSTL( specs.outside_point, specs.stls[stli] );
                    amrex::Print() << "Num particles after stl removal "
                                   << bpc.TotalNumberOfParticles() << "\n";
                }
            }
...
#include "BDEM_Verlet_integration.H"
        ...
        for ( int stli = 0; stli < specs.stls.size(); stli++ )
        {
            ...
            if ( specs.stls[stli].dynamicstl != 0 )
            {
                ...
                {
                    specs.stls[stli].stlptr->move_stl(
                        dt,
                        specs.stls[stli].dynamicstl,
                        specs.stls[stli].dynstl_transl_dir.data(),
                        specs.stls[stli].dynstl_center.data(),
                        specs.stls[stli].dynstl_transl_vel );
                }
                else if ( specs.stls[stli].dynamicstl == 2 )
                {
                    specs.stls[stli].stlptr->move_stl(
                        dt,
                        specs.stls[stli].dynamicstl,
                        specs.stls[stli].dynstl_rot_dir.data(),
                        specs.stls[stli].dynstl_center.data(),
                        specs.stls[stli].dynstl_rot_vel );
                }
                else if ( specs.stls[stli].dynamicstl == 3 )
                {
                    amrex::Real stl_pos_prv = specs.stls[stli].stl_vib_amp *
                                              sin( ( time + specs.stls[stli].stl_timeoffset ) *
                                                   specs.stls[stli].stl_vib_freq );
                    amrex::Real stl_pos_nxt = specs.stls[stli].stl_vib_amp *
                                              sin( ( time + dt + specs.stls[stli].stl_timeoffset ) *
                                                   specs.stls[stli].stl_vib_freq );
                    amrex::Real stl_vel = ( stl_pos_nxt - stl_pos_prv ) / dt;
                    specs.stls[stli].stlptr->move_stl(
                        dt,
                        specs.stls[stli].dynamicstl,
                        specs.stls[stli].stl_vib_dir.data(),
                        specs.stls[stli].dynstl_center.data(),
                        stl_vel );
                }
                else
                ...
                if ( steps % specs.num_redist == 0 )
                {
                    ...
                    specs.stls[stli].stlptr->orb_of_triangulation(
                        0,
                        specs.stls[stli].stlptr->num_tri - 1,
                        0,
                        specs.stls[stli].stlptr->sorted_indexarray );
                }
            }
        }
        ...
        if ( output_time > specs.write_output_time )
        {
            ...
            output_it++;
            bpc.writeParticles(
                output_it + specs.stepoffset, specs.bonded_sphere_particles, specs.pltprefix );
            const std::string &rstfile =
                specs.pltprefix + amrex::Concatenate( "rst", output_it + specs.stepoffset, 5 );
            ...
            for ( int stli = 0; stli < specs.stls.size(); stli++ )
            {
                ...
                std::string stlpltfile = amrex::Concatenate(
                    ( specs.stls[stli].name + "_" ).c_str(), output_it + specs.stepoffset, 5 );
                if ( specs.stls[stli].dynamicstl != 0 )
                {
                    specs.stls[stli].stlptr->write_stl_file( stlpltfile );
            ...
            if ( specs.calc_mass_flow )
            {
                ...
                t_prev       = time;
                bpc.Calculate_Total_Mass_MaterialPoints(
                    mass_flow_next, specs.mass_flow_dir, specs.mass_flow_cutoff );
                Real flow_out  = mass_flow_prev - mass_flow_next;
                mass_flow_prev = mass_flow_next;
                PrintToFile( specs.pltprefix + "Mass_Flow.out" )
                    << time << "\t" << delta_t << "\t" << flow_out << "\t" << mass_flow_next
                    << "\n";
            }
    ...
    bpc.redist_particles( 0, specs.using_softwalls );
    bpc.writeParticles(
        output_it + 1 + specs.stepoffset, specs.bonded_sphere_particles, specs.pltprefix );
    const std::string &rstfile =
        specs.pltprefix + amrex::Concatenate( "rst", output_it + 1 + specs.stepoffset, 5 );
    bpc.Checkpoint( rstfile, "particles" );
    if ( specs.using_softwalls )
    {
        bpc.reassignParticles_softwall();
