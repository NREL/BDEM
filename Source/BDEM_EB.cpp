...
void make_conecup_levelset(
    const Geometry &geom, const BoxArray &ba, const DistributionMapping &dm )
{
    ...
    auto conecup =
        EB2::translate( EB2::lathe( EB2::makeUnion( cyl1, frustum1, frustum2 ) ), center );
    ...
    // Build EB
    EB2::Build( conecup_gshop, geom_ls, required_coarsening_level, max_coarsening_level );
    ...
    // build factory
    ebfactory =
        new EBFArrayBoxFactory( eblev, geom, ba, dm, { nghost, nghost, nghost }, EBSupport::full );
...
void make_hopper_levelset( const Geometry &geom, const BoxArray &ba, const DistributionMapping &dm )
{
    ...
    // Build EB
    EB2::Build( hopper_gshop, geom_ls, required_coarsening_level, max_coarsening_level );
...
void make_wedge_hopper_levelset(
    const Geometry &geom, const BoxArray &ba, const DistributionMapping &dm )
{
    int ls_ref = ls_refinement;
    // Define nGrow of level-set and EB
...
void make_silo_levelset( const Geometry &geom, const BoxArray &ba, const DistributionMapping &dm )
{
    ...
    amrex::Print() << " Center:    " << center[0] << ", " << center[1] << ", " << center[2]
                   << std::endl;
    amrex::Print() << " blo:    " << blo[0] << ", " << blo[1] << ", " << blo[2] << std::endl;
    amrex::Print() << " bhi:    " << bhi[0] << ", " << bhi[1] << ", " << bhi[2] << std::endl;
    ...
    // Build EB
    EB2::Build( slotcyl_gshop, geom_ls, required_coarsening_level, max_coarsening_level );
