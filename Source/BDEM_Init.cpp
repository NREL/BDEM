...
void BDEMParticleContainer::InitParticles(
    ...
    const int solve_fibrils )
{
    ...
    if ( ParallelDescriptor::IOProcessor() )
    {
        ...
        int p_types[BP_TYPES] = { BP_NP0,  BP_NP1,  BP_NP2,  BP_NP3,  BP_NP4,  BP_NP5,
                                  BP_NP6,  BP_NP7,  BP_NP8,  BP_NP9,  BP_NP10, BP_NP11,
                                  BP_NP12, BP_NP13, BP_NP14, BP_NP15, BP_NP16, BP_NP17,
                                  BP_NP18, BP_NP19, BP_NP20, BP_NP21, BP_NP22, BP_NP23,
                                  BP_NP24, BP_NP25, BP_NP26, BP_NP27, BP_NP28 };
        ...
        for ( int i = 0; i < np; i++ )
        {
            ...
            // Unique id for each particle across all procs created using id, cpu, and Cantor
            // pairing function
            p.idata( intData::unique_id ) =
                (int)( ( p.id() + p.cpu() ) * ( p.id() + p.cpu() + 1 ) + p.cpu() );
            ...
            if ( do_heat_transfer )
            {
                ...
                // Overwrite temperature in file if a valid mean temperature and temp stdev are
                // specified
                if ( temp_mean > 0.0 && temp_stdev > 0.0 )
                {
                    p.rdata( realData::temperature ) =
                        max( amrex::RandomNormal( temp_mean, temp_stdev ), 1.0 );
                }
            }
            else
            {
            ...
            // NOTE: Calculation of particle mass for glued sphere particles assumes no component
            // sphere overlap
            p.rdata( realData::volume ) =
                fourbythree * PI * pow( p.rdata( realData::radius ), three );
            p.rdata( realData::mass ) = p.rdata( realData::density ) * p.rdata( realData::volume );
            p.rdata( realData::Iinv ) =
                2.5 / ( p.rdata( realData::mass ) * pow( p.rdata( realData::radius ), two ) );

            // User input for angular velocity when using glued sphere particles (at least for
            // testing purposes)
            ...
            for ( int br = 0; br < MAXBRIDGES; br++ )
                p.idata( intData::first_bridge + br ) = -1;

            // If using liquid bridging, calculate particle liquid and recalculate particle mass and
            // density
            if ( liquid_bridging )
            {
                Real MC = ( MC_stdev > 0.0 )
                              ? min( max( amrex::RandomNormal( MC_avg, MC_stdev ), 0.0 ), 0.9 )
                              : MC_avg;
                p.rdata( realData::liquid_volume ) =
                    ( MC > FSP ) ? ( p.rdata( realData::density ) * p.rdata( realData::volume ) /
                                     liquid_density ) *
                                       ( MC - FSP ) / ( 1 - MC )
                                 : 0;
                p.rdata( realData::mass ) = p.rdata( realData::density ) *
                                            p.rdata( realData::volume ) *
                                            ( 1.0 + MC / ( 1.0 - MC ) );
            ...
            for ( int b = 0; b < MAXBONDS; b++ )
                p.idata( intData::first_bond + b ) = -1;
...
void BDEMParticleContainer::InitBondedParticles(
    const std::string &filename,
    bool &do_heat_transfer,
    int contact_law,
    ...
    Real FSP )
{
    ...
    if ( ParallelDescriptor::IOProcessor() )
    {
        ...
        int bp_types[BP_TYPES]         = { BP_NP0,  BP_NP1,  BP_NP2,  BP_NP3,  BP_NP4,  BP_NP5,
                                           BP_NP6,  BP_NP7,  BP_NP8,  BP_NP9,  BP_NP10, BP_NP11,
                                           BP_NP12, BP_NP13, BP_NP14, BP_NP15, BP_NP16, BP_NP17,
                                           BP_NP18, BP_NP19, BP_NP20, BP_NP21, BP_NP22, BP_NP23,
                                           BP_NP24, BP_NP25, BP_NP26, BP_NP27, BP_NP28 };
        ...
        for ( int i = 0; i < np; i++ )
        {
            ...
            amrex::Real bp_q[4] = {
                cos( eaz / 2.0 ) * cos( eay / 2.0 ) * cos( eax / 2.0 ) +
                    sin( eaz / 2.0 ) * sin( eay / 2.0 ) * sin( eax / 2.0 ),
                -sin( eaz / 2.0 ) * sin( eay / 2.0 ) * cos( eax / 2.0 ) +
                    cos( eaz / 2.0 ) * cos( eay / 2.0 ) * sin( eax / 2.0 ),
                cos( eaz / 2.0 ) * sin( eay / 2.0 ) * cos( eax / 2.0 ) +
                    sin( eaz / 2.0 ) * cos( eay / 2.0 ) * sin( eax / 2.0 ),
                sin( eaz / 2.0 ) * cos( eay / 2.0 ) * cos( eax / 2.0 ) -
                    cos( eaz / 2.0 ) * sin( eay / 2.0 ) * sin( eax / 2.0 ) };
            ...
            for ( int j = 0; j < bp_types[bp_type]; j++ )
            {
                ...
                bp_unique_ids[j] =
                    (int)( ( bp_ids[j] + cpu_id ) * ( bp_ids[j] + cpu_id + 1 ) + cpu_id );
            }
            for ( int j = 0; j < bp_types[bp_type]; j++ )
            {
                ParticleType p;
                ...
                if ( cantilever_beam_test && j == bp_types[bp_type] - 1 )
                    bp_phase = -1; // Left-most particle is held inert
...
void BDEMParticleContainer::InitChemSpecies(
    int ndomains, Real *mincoords, Real *maxcoords, Real *spec_massfracs )
{
    const int lev        = 0;
    const Geometry &geom = Geom( lev );
    const auto plo       = Geom( lev ).ProbLoArray();
    ...
    for ( MFIter mfi = MakeMFIter( lev ); mfi.isValid(); ++mfi )
    {
        ...
        amrex::ParallelFor( np, [=] AMREX_GPU_DEVICE( int i ) noexcept {
            ...
            {
                if ( x > mincoords[d * AMREX_SPACEDIM + 0] &&
                     x <= maxcoords[d * AMREX_SPACEDIM + 0] &&
                     y > mincoords[d * AMREX_SPACEDIM + 1] &&
                     y <= maxcoords[d * AMREX_SPACEDIM + 1] &&
                     z > mincoords[d * AMREX_SPACEDIM + 2] &&
                     z <= maxcoords[d * AMREX_SPACEDIM + 2] )
                {
                    for ( int sp = 0; sp < MAXSPECIES; sp++ )
                    {
...
void BDEMParticleContainer::removeParticlesOutsideBoundary(
    const MultiFab *lsmfab, const EBFArrayBoxFactory *ebfactory, const int ls_refinement )
{
    const int lev                        = 0;
    auto &plev                           = GetParticles( lev );
...
void BDEMParticleContainer::removeParticlesInsideSTL(
    Vector<Real> outside_point, stl_specs &stlspcs )
{
    ...
    amrex::Print() << "STL BBs: " << stlptr->bbox_lo[0] << " " << stlptr->bbox_hi[0] << " "
                   << stlptr->bbox_lo[1] << " " << stlptr->bbox_hi[1] << " " << stlptr->bbox_lo[2]
                   << " " << stlptr->bbox_hi[2] << "\n";
    ...
    for ( MFIter mfi = MakeMFIter( lev ); mfi.isValid(); ++mfi )
    {
            ...
            amrex::ParallelFor( np, [=] AMREX_GPU_DEVICE( int i ) noexcept {
                     ...
                     ( p.pos( 2 ) - p.rdata( realData::radius ) < z_hi_bb ) )
                {
                    ...
                    brutesearch_with_intersections(
                        num_tri, ploc, mindist, min_tri_id, outp, num_intersects, tri_pts );
        ...
        else
        {
            ...
            Array<Real, 3> pi_arr{
                stlspcs.point_inside[0], stlspcs.point_inside[1], stlspcs.point_inside[2] };
            Array<Real, 3> di_arr{
                stlspcs.direction_inside[0],
                stlspcs.direction_inside[1],
                stlspcs.direction_inside[2] };
            int *tris_per_cell = stlptr->tris_per_cell;
            Array<Real, 3> gridbbmin_arr{
                stlptr->grid.bbmin[0], stlptr->grid.bbmin[1], stlptr->grid.bbmin[2] };
            Array<Real, 3> griddelta_arr{
                stlptr->grid.delta[0], stlptr->grid.delta[1], stlptr->grid.delta[2] };
            Array<int, 3> gridsize_arr{
                stlptr->grid.size[0], stlptr->grid.size[1], stlptr->grid.size[2] };
        ...
        amrex::ParallelFor( np, [=] AMREX_GPU_DEVICE( int i ) noexcept {
                 ...
                 ( p.pos( 2 ) - p.rdata( realData::radius ) < z_hi_bb ) )
            {
                ...
                if ( num_intersects % 2 == 1 )
                {
                    ...
                    amrex::Print() << "particle inside stl:" << ploc[0] << "\t" << ploc[1] << "\t"
                                   << ploc[2] << "\n";
#else
...
void BDEMParticleContainer::reassignParticleProperties(
    Real reinit_rad, Real reinit_dens, Real reinit_E, Real reinit_nu )
{
    const int lev        = 0;
    auto &plev           = GetParticles( lev );
    const Geometry &geom = Geom( lev );
...
void BDEMParticleContainer::InitParticles(
    Real mincoords[THREEDIM],
    Real maxcoords[THREEDIM],
    Real meanvel[THREEDIM],
    Real fluctuation[THREEDIM],
    Real rad,
    Real dens,
    Real E,
    Real nu,
    ...
    Real FSP )
{
    ...
    int p_types[BP_TYPES]         = {
                BP_NP0,  BP_NP1,  BP_NP2,  BP_NP3,  BP_NP4,  BP_NP5,  BP_NP6,  BP_NP7,  BP_NP8,  BP_NP9,
                BP_NP10, BP_NP11, BP_NP12, BP_NP13, BP_NP14, BP_NP15, BP_NP16, BP_NP17, BP_NP18, BP_NP19,
                BP_NP20, BP_NP21, BP_NP22, BP_NP23, BP_NP24, BP_NP25, BP_NP26, BP_NP27, BP_NP28 };
        ...
        for ( IntVect iv = tile_box.smallEnd(); iv <= tile_box.bigEnd(); tile_box.next( iv ) )
        {
            if ( do_multi_part_per_cell == 0 || layer_particles > 0 )
            {
                ...
                for ( int pl = 0; pl < layers; pl++ )
                {
                    ...
                    y = ( layers > 1 )
                            ? plo[YDIR] + ( iv[YDIR] + ( ( 1.0 + pl ) / ( 1.0 + layers ) ) ) * dy
                            : plo[YDIR] + ( iv[YDIR] + dist( mt ) ) * dy;
                    z = plo[ZDIR] + ( iv[ZDIR] + dist( mt ) ) * dz;

                    if ( x >= mincoords[XDIR] && x <= maxcoords[XDIR] && y >= mincoords[YDIR] &&
                         y <= maxcoords[YDIR] && z >= mincoords[ZDIR] && z <= maxcoords[ZDIR] )
                    {
                        Real MC = 0.0;
                        if ( liquid_bridging )
                            MC = ( MC_stdev > 0.0 )
                                     ? min( max( amrex::RandomNormal( MC_avg, MC_stdev ), 0.0 ),
                                            0.9 )
                                     : MC_avg;
                        if ( bonded_sphere_particles )
                        {
                            int type =
                                ( p_type == -1 ) ? ceil( amrex::Random() * BP_TYPES ) - 1 : p_type;
                            if ( type_list.size() > 0 )
                            {
                                if ( use_type_dist )
                                {
                                    ...
                                    while ( !found_type )
                                    {
                                        ...
                                        {
                                            type_idx++;
                                            if ( type_idx >= type_list.size() )
                                                amrex::Abort( "Something went wrong with type dist "
                                                              "calculations!\n" );
                                        }
                                    }
                                ...
                                {
                                    type =
                                        type_list[ceil( amrex::Random() * type_list.size() ) - 1];
                                }
                                if ( type < 0 || type > BP_TYPES - 1 )
                            ...
                            {
                                // Unique id for each particle across all procs created using id,
                                // cpu, and Cantor pairing function
                                bp_ids[j]  = ParticleType::NextID();
                                int cpu_id = ParallelDescriptor::MyProc();
                                bp_unique_ids[j] =
                                    (int)( ( bp_ids[j] + cpu_id ) * ( bp_ids[j] + cpu_id + 1 ) + cpu_id );
                        ...
                        {
                            // Calculate radius using random distribution if min and max values are
                            // specified
                            Real p_rad = ( min_rad > 0.0 && max_rad > 0.0 && max_rad > min_rad )
                                             ? min_rad + ( max_rad - min_rad ) * amrex::Random()
                                             : rad;
                    ...
                    for ( int j = 0; j < 2; j++ )
                    {
                        for ( int i = 0; i < 2; i++ )
                        {
                                 ...
                                 z >= mincoords[ZDIR] and z <= maxcoords[ZDIR] )
                            {
                                ...
                                if ( liquid_bridging )
                                    MC = ( MC_stdev > 0.0 )
                                             ? min( max( amrex::RandomNormal( MC_avg, MC_stdev ),
                                                         0.0 ),
                                                    0.9 )
                                             : MC_avg;
                                if ( bonded_sphere_particles )
                                {
                                    ...
                                    if ( type_list.size() > 0 )
                                    {
                                        if ( use_type_dist )
                                        {
                                            ...
                                            while ( !found_type )
                                            {
                                                ...
                                                else
                                                {
                                                    type_idx++;
                                                    if ( type_idx >= type_list.size() )
                                                        amrex::Abort( "Something went wrong with "
                                                                      "type dist calculations!\n" );
                                                }
                                            }
                                            type = type_list[type_idx];
                                        }
                                        ...
                                        {
                                            type = type_list
                                                [ceil( amrex::Random() * type_list.size() ) - 1];
                                        }
                                        if ( type < 0 || type > BP_TYPES - 1 )
                                            Abort( "\nInvalid particle type specified .\n" );
                                    }
                                    ...
                                    amrex::Real quats[4] = {
                                        ...
                                        cos( eaz / 2.0 ) * sin( eay / 2.0 ) * cos( eax / 2.0 ) +
                                            sin( eaz / 2.0 ) * cos( eay / 2.0 ) * sin( eax / 2.0 ),
                                        sin( eaz / 2.0 ) * cos( eay / 2.0 ) * cos( eax / 2.0 ) -
                                            cos( eaz / 2.0 ) * sin( eay / 2.0 ) *
                                                sin( eax / 2.0 ) };
                                    Real bp_vel[THREEDIM] = {
                                        meanvel[XDIR] +
                                            fluctuation[XDIR] * ( amrex::Random() - half ),
                                        meanvel[YDIR] +
                                            fluctuation[XDIR] * ( amrex::Random() - half ),
                                        meanvel[ZDIR] +
                                            fluctuation[XDIR] * ( amrex::Random() - half ) };
                                    ...
                                    {
                                        // Unique id for each particle across all procs created
                                        // using id, cpu, and Cantor pairing function
                                        bp_ids[pi] = ParticleType::NextID();
                                        int cpu_id = ParallelDescriptor::MyProc();
                                        bp_unique_ids[pi] =
                                            (int)( ( bp_ids[pi] + cpu_id ) * ( bp_ids[pi] + cpu_id + 1 ) + cpu_id );
                                    }
                                    for ( int pi = 0; pi < p_types[type]; pi++ )
                                    {
                                        ParticleType p;
                                        ...
                                        get_bonded_particle_pos(
                                            type, pi, rad, bp_pos, quats, pc_pos );
                                        bp_init(
                                            p,
                                            p_data,
                                            bp_phase,
                                ...
                                {
                                    // Calculate radius using random distribution if min and max
                                    // values are specified
                                    Real p_rad =
                                        ( min_rad > 0.0 && max_rad > 0.0 && max_rad > min_rad )
                                            ? min_rad + ( max_rad - min_rad ) * amrex::Random()
                                            : rad;
                                    ...
                                    ParticleType p = generate_particle(
                                        ...
                                        meanvel[XDIR] +
                                            fluctuation[XDIR] * ( amrex::Random() - half ),
                                        meanvel[YDIR] +
                                            fluctuation[YDIR] * ( amrex::Random() - half ),
                                        meanvel[ZDIR] +
                                            fluctuation[ZDIR] * ( amrex::Random() - half ),
                                        dens,
                                        p_rad,
                                        E,
                                        nu,
...
BDEMParticleContainer::ParticleType BDEMParticleContainer::generate_particle(
    Real x,
    Real y,
    Real z,
    Real velx,
    ...
    int p_type )
{
    ...
    if ( MC > 0.0 )
    {
        p.rdata( realData::liquid_volume ) =
            ( MC > FSP )
                ? ( p.rdata( realData::density ) * p.rdata( realData::volume ) / liquid_density ) *
                      ( MC - FSP ) / ( 1 - MC )
                : 0;
        p.rdata( realData::mass ) = p.rdata( realData::density ) * p.rdata( realData::volume ) *
                                    ( 1.0 + MC / ( 1.0 - MC ) );
