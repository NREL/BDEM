...
void STLtools::buildGridData( int gsize[3] )
{
    ...
    for ( int tri = 0; tri < num_tri; tri++ )
    {
        ...
        for ( int cell = 0; cell < grid.numberOfCells; cell++ )
        {
            ...
            Real distMag =
                point_tri_distance( cellCentroidCoord, A, B, C, closestType, cellCentroidDist );

            if ( std::fabs( cellCentroidDist[0] ) < 0.5 * grid.delta[0] + 1e-16 &&
...
#define WRITE_STL_GRID
    ...
    // Write dimensions
    gridFile << "DIMENSIONS " << grid.size[0] + 1 << " " << grid.size[1] + 1 << " "
             << grid.size[2] + 1 << "\n";
...
void STLtools::orb_of_triangulation( int startindex, int endindex, int dir, int *indexarray )
{
...
AMREX_GPU_HOST_DEVICE void STLtools::brutesearch(
    int startindex, int endindex, int *indexarray, Real p[3], Real &mindist, int &idmin ) const
{
    mindist = 1e50;
    idmin   = -1;
...
Real STLtools::getNormalComponent( int id, Real p[3] )
{
    ...
    Real vec_dot_n =
        ( p[0] - cent[0] ) * n[0] + ( p[1] - cent[1] ) * n[1] + ( p[2] - cent[2] ) * n[2];
...
void STLtools::searchtriangulation(
    int startindex,
    int endindex,
    int *indexarray,
    Real p[3],
...
void STLtools::writeVTK( std::string fname ) const
{
    ...
    for ( int i = 0; i < num_tri; i++ )
    {
        vtkFile << "3 " << vtk_triangles[i].node_id[0] << " " << vtk_triangles[i].node_id[1] << " "
    ...
    for ( int i = 0; i < num_tri; i++ )
    {
        vtkFile << shear_stress[3 * i] << " " << shear_stress[3 * i + 1] << " "
                << shear_stress[3 * i + 2] << "\n";
    }
...
void STLtools::init_vtk_data()
{
    ...
    for ( size_t tri = 0; tri < num_tri; tri++ )
    {
        ...
        for ( size_t n = 0; n < 3; n++ )
        {
            ...
            {
                // Check triplet
                if ( std::fabs( vtk_points[nId].x - p[0] ) < 1e-12 &&
...
void STLtools::move_stl(
    Real timestep, int movetype, amrex::Real movedir[3], amrex::Real movecenter[3], Real movevel )
{
...
void STLtools::printForces() const
{
    ...
    Print() << "pressure = " << ptot << "   shear stress = ( " << sstot[0] << " " << sstot[1] << " "
            << sstot[2] << " )\n";
}
