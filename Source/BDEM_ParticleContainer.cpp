...
Real BDEMParticleContainer::compute_coll_timescale( int bonded_sphere_particles, int contact_law )
{
    ...
    coll_timescale = amrex::ReduceMin( *this, [=] AMREX_GPU_HOST_DEVICE( const PType &p ) -> Real {
        ...
        if ( contact_law == 1 )
        {
            ...
            Real temp =
                2.0 * E2 * ( 2.0 - nu1 ) * ( 1.0 + nu1 ) + 2.0 * E1 * ( 2.0 - nu2 ) * ( 1.0 + nu2 );
        ...
        if ( bonded_sphere_particles )
            bonded_dt = 0.8165 * 2.0 * p.rdata( realData::radius ) *
                        pow( p.rdata( realData::density ) / DEM::E_bond, 0.5 );

        return ( std::min(
            bonded_dt,
            std::pow(
                k_n / ( p.rdata( realData::mass ) ) * PI * PI /
                    ( PI * PI + log( DEM::e_n ) * log( DEM::e_n ) ),
                -half ) ) );
    } );
...
void BDEMParticleContainer::computeForces(
    Real &dt,
    ...
    bool stl_geom_present,
    int contact_law,
    int steps,
    RealVect &gravity,
    amrex::Vector<stl_specs> &stls,
    Real time,
    const int bonded_sphere_particles,
    const int liquid_bridging,
    const int particle_cohesion,
    const Real init_force,
    const int init_force_dir,
    const int init_force_comp,
    const Real cb_force,
    const Real cb_torq,
    const int cb_dir,
...
)
{
    ...
    for ( int stli = 0; stli < stls.size(); stli++ )
    {
        ...
        amrex::ParallelFor( stlptr->num_tri, [=] AMREX_GPU_DEVICE( int i ) {
            pressure[i]             = 0.;
            shear_stress[3 * i]     = 0.;
    ...
    const amrex::Real &dt, int do_chemistry, Real minradfrac, int verlet_scheme, int adapt_step )
{
    ...
    for ( MFIter mfi = MakeMFIter( lev ); mfi.isValid(); ++mfi )
    {
        ...
        amrex::ParallelFor( np, [=] AMREX_GPU_DEVICE( int i ) noexcept {
            ...
            { // Particles with phase = -1 are inert and do not move
                Real pos_old[3];
                Real pos_new[3];
                ...
                if ( do_chemistry )
                {
                    ...
                    for ( int sp = 0; sp < nspecies; sp++ )
                    {
                        // conc in kg/m3
                        spec[sp] =
                            p.rdata( realData::firstspec + sp ) * p.rdata( realData::density );
                    }
                    ...
                    for ( int sp = 0; sp < nspecies; sp++ )
                    {
                        p.rdata( realData::firstspec + sp ) +=
                            wdot[sp] * dt / p.rdata( realData::density );
                    }
...
void BDEMParticleContainer::clearSourcingVolume(
    Real mincoords[THREEDIM], Real maxcoords[THREEDIM] )
{
    const int lev = 0;
    auto &plev    = GetParticles( lev );
    ...
    for ( MFIter mfi = MakeMFIter( lev ); mfi.isValid(); ++mfi )
    {
        ...
        amrex::ParallelFor( np, [=] AMREX_GPU_DEVICE( int i ) noexcept {
            ...
            if ( x + rad >= mincoords_x && x - rad <= maxcoords_x && y + rad >= mincoords_y &&
                 y - rad <= maxcoords_y && z + rad >= mincoords_z && z - rad <= maxcoords_z )
            {
                p.id() = -1;
            }
        } );
...
void BDEMParticleContainer::computeMoistureContent(
    Real MC_avg, Real MC_stdev, Real liquid_density, Real FSP )
{
    ...
    for ( MFIter mfi = MakeMFIter( lev ); mfi.isValid(); ++mfi )
    {
        ...
        amrex::ParallelFor( np, [=] AMREX_GPU_DEVICE( int i ) noexcept {
            ParticleType &p = pstruct[i];
            // Real MC = (MC_stdev > 0.0) ? std::min(std::max(amrex::RandomNormal(MC_avg,
            // MC_stdev),0.0),0.9):MC_avg;
            Real MC          = MC_avg;
            Real original_lv = p.rdata( realData::liquid_volume );
            p.rdata( realData::liquid_volume ) =
                ( MC > FSP || MC == 0.0 ) ? ( p.rdata( realData::density ) *
                                              p.rdata( realData::volume ) / liquid_density ) *
                                                ( MC - FSP ) / ( 1.0 - MC )
                                          : 0;
            ...
            if ( original_lv > 0.0 )
                p.rdata( realData::total_bridge_volume ) *=
                    p.rdata( realData::liquid_volume ) / original_lv;
...
void BDEMParticleContainer::Calculate_Total_Mass_MaterialPoints(
    Real &total_mass, int cdir, Real cutoff )
{
    const int lev        = 0;
    const Geometry &geom = Geom( lev );
    auto &plev           = GetParticles( lev );
    ...
    using PType = typename BDEMParticleContainer::SuperParticleType;
    total_mass  = amrex::ReduceSum( *this, [=] AMREX_GPU_HOST_DEVICE( const PType &p ) -> Real {
...
void BDEMParticleContainer::Calculate_Total_Speed_MaterialPoints( Real &total_speed )
{
    ...
    using PType = typename BDEMParticleContainer::SuperParticleType;
    total_speed = amrex::ReduceSum( *this, [=] AMREX_GPU_HOST_DEVICE( const PType &p ) -> Real {
        Real pvel[3] = {
            p.rdata( realData::xvel ), p.rdata( realData::yvel ), p.rdata( realData::zvel ) };
...
void BDEMParticleContainer::writeParticles(
    const int n, const int bonded_sphere_particles, const std::string pltprefix )
{
...
void BDEMParticleContainer::removeEBOverlapParticles(
    EBFArrayBoxFactory *eb_factory, const MultiFab *lsmfab, const int ls_refinement )
{
    // This function removes particles that have gotten "wedged" in corners of an EB
    // boundary (where forces are not calculated correctly, and particles may stick out of the
    // boundary)
        ...
        for ( MFIter mfi = MakeMFIter( lev ); mfi.isValid(); ++mfi )
        {
            ...
            if ( ( *flags )[mfi].getType( amrex::grow( bx, 1 ) ) == FabType::singlevalued )
            {
                ...
                amrex::ParallelFor( np, [=] AMREX_GPU_DEVICE( int i ) noexcept {
                    ...
                    Real ls_value =
                        get_levelset_value( ppos_inert, ls_refinement, phiarr, plo, dx );
