...
set size square
set output 'beam_displacement.tex'
  
set xrange[0:8]
set yrange[*:0.0]
   
set xlabel '$\widetilde{F}_{t}^{0}$'
set ylabel '$\widetilde{y}^{0}$' offset 8,12.0 rotate by 0
...
p f1 u ((-1.0)*(($1)*(L-rs)**2)/(E*I)):(($2)/(L-rs)) with points ls 1 title 'BDEM',\
