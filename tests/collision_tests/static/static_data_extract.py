...
if __name__ == "__main__":
    ...
    for i, fn in enumerate(fn_list):
        ...
        overlap_n = rdata[:,17]   # assumes 3D
        fn_y = rdata[:,18]
        vr_n = rdata[:,19]
        overlap_t_y = rdata[:,20]
        ft_y = rdata[:,21]
        vr_t_y = rdata[:,22]
        # outfile.write("%e\t%e\t%e\t%e\n"%(timestamp*out_time,overlap_n[0],fn_y[0],vr_n[0]))
        outfile.write("%e\t%e\t%e\t%e\t%e\t%e\t%e\n"%(timestamp*out_time,overlap_n[0],fn_y[0],vr_n[0],overlap_t_y[0],ft_y[0],vr_t_y[0]))
